...
# prepare the dataset for validating ProM classifier

load("/omics/odcf/analysis/OE0167_projects/dachs_genetic_data_platform/dachs_cohort/PMunicox.RData")
multipm_impu <- vector(n_impu,mode="list")

for (i in 1:n_impu) {
  
  multipm_impu[[i]]<- multi_impu[[i]][(multi_impu[[i]]$id %in%  unicoxpm$id ), ]
...
save(multipm_impu, file = '/omics/odcf/analysis/OE0167_projects/dachs_genetic_data_platform/dachs_cohort/multicoxPM_impu20list.RData')
