...
cpg_orig<-cpg_orig[!is.na(cpg_orig)]
cpg_unique<-unique(cpg_orig) ## 200
save(cpg_unique, cpg_orig, file = 'cpg_validate.RData')
rm(cpg_orig)
...
intersect<-Reduce(intersect, list(cpg_unique, methy)) # all the 200 CpGs were available 
...
beta_vali<-subset(beta_vali, case=='TU') ## 2391 cases
...
beta_all<-subset(beta_all, !(beta_all$uid %in% excl)) ### 2316
